...
if [ "$(command -v node)" ] && [ "$(node -v | cut -d 'v' -f 2 | cut -d '.' -f 1)" -ge 16 ] && [ ! -f "${lgsmdir}/node_modules/gamedig/bin/gamedig.js" ]; then
	...
	cd "${lgsmdir}" || exit
	curl -L -o package.json "https://raw.githubusercontent.com/thefoofighter/LinuxGSM/${githubbranch}/package.json"
	npm install
elif [ "$(command -v node)" ] && [ "$(node -v | cut -d 'v' -f 2 | cut -d '.' -f 1)" -ge 16 ]; then
	cd "${lgsmdir}" || exit
	curl -s -L -o package.json "https://raw.githubusercontent.com/thefoofighter/LinuxGSM/${githubbranch}/package.json"
